...
err_t Isorropia::Init() {
	LOG("Isorropia. Created and developed by xwize 2018 - 2019.\n");
...
void Isorropia::PrePresent(IDXGISwapChain* pSwapChain, UINT syncInterval, UINT* pSyncIntervalOut) {
	...
	// Force no sync, we can force what we want in the control panel
	*pSyncIntervalOut = 0;
}

void Isorropia::PostPresent() {

	// Better not try to do anything until the patch worked
	if (_AdaptiveAATargetFPSPtr() == NULL)
		return;
	...
	if (frame % ISORROPIA_FRAMES_PER_PRINT == 0) {
		...
		DebugPrintf("Frame: %d. GPU Usage: %d [%d,%d]. FT: %f. TFPS: %.2f\n",
			frame, gpuPerfMonitor.Usage(), usagesMin, usagesMax, frameTimeMS, AdaptiveAATargetFPS());
#endif
	}
	frame++;
}
...
void Isorropia::_Control() {
	...
	const float AATargetMd = ISORROPIA_TARGET_FPS;
	const float AATargetHi = 1.0f;

	const float AATargets[numSetPoints] = { AATargetLo, AATargetMd, AATargetHi };
